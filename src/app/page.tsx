...
export default function Home() {
  ...
  const [topPosts, setTopPosts] = useState<Array<{ id: string; title: string; tags?: string[] }>>([])
  const [commits, setCommits] = useState<Array<{ sha: string; message: string; date: string; url: string; author: string }>>([])
  ...
  useEffect(() => {
    ...
    // Fetch top posts from API to get tags
    const fetchTopPosts = async () => {
      try {
        const response = await fetch('/api/posts')
        if (response.ok) {
          const posts = await response.json()
          setTopPosts(posts.slice(0, 2))
        } else {
        ...
        try {
          ...
          console.error('Error fetching from posts.json:', e)
        }
      }
    }
    fetchTopPosts()
        ...
        <div className="flex-1 flex flex-col lg:h-screen">
          ...
          <section className="mb-8">
            ...
            <a href="https://github.com/parasxparkash" target="_blank" rel="noopener noreferrer" className="block">
              <Image
                ...
                alt="GitHub contributions chart showing Paras Parkash's coding activity for 2025"
                width={800}
                height={200}
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
           <div className="opacity-0 transform animate-fade-in animate-delay-200 sticky top-0 bg-white dark:bg-zinc-900 z-10">
             <div className="text-muted-foreground inline-flex h-9 w-full items-center justify-start rounded-lg mb-2 md:mb-4 border-none bg-transparent p-0 overflow-x-auto scrollbar-hide">
               {tabs.map((tab) => (
                 <button
                   ...
                   type="button"
                  className={`tab-button inline-flex h-[calc(100%-1px)] flex-1 min-w-0 items-center justify-center gap-1.5 rounded-md border border-transparent px-3 py-1 text-sm font-medium whitespace-nowrap !bg-transparent !border-none !shadow-none !font-light transition-all duration-300 ease-out hover:!font-bold ${
                    activeTab === tab.id
            ...
            <div className="pl-4 md:pl-6 pb-12">
              {renderTabContent()}
            </div>
          </div>
        </div>
